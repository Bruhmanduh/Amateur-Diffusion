...
class VAE_encoder(nn.Sequential):
    def __init__(self):
        super().__init(
            # (batch size, 3, 512, 512) --> (batch size, 128, 512, 512)
            nn.Conv2d(3, 128, kernel_size=3, padding=1),

            # (batch size, 128, 512, 512) --> no change in image shape
    ...
    def forward(self, x: torch.Tensor, noise: torch.Tensor) -> torch.Tensor:
        ...
        # noise: (batch_size, 8, 64, 64)
        for layer in self:
            if getattr(layer, 'stride', None) == (2, 2):
                x = F.pad(x, (0, 1, 0, 1))
            x = layer(x)

        # (batch_size, 8, 64, 64) --> 2 tensors of (batch_size, 4, 64, 64)
        mean, log_variance =  torch.chunk(x, 2, dim=1)

        # 2 tensors of (batch_size, 4, 64, 64)
        log_variance = torch.clamp(log_variance, -30, 20)
        ...
        # Z = N(0, 1)  X = N(mean, variance) --> X = mean + stdev * Z
